...
// Job applications — safely handle if table doesn't exist yet
$job_count = 0;
$job_new   = 0;
$job_res   = $conn->query("SHOW TABLES LIKE 'job_applications'");
if ($job_res && $job_res->num_rows > 0) {
    $job_count = (int)($conn->query("SELECT COUNT(*) AS c FROM job_applications")->fetch_assoc()['c'] ?? 0);
    $job_new   = (int)($conn->query("SELECT COUNT(*) AS c FROM job_applications WHERE status='new'")->fetch_assoc()['c'] ?? 0);
}
...
$recent_jobs   = null;
if ($job_count > 0) {
    $recent_jobs = $conn->query("SELECT full_name, position, status, created_at FROM job_applications ORDER BY id DESC LIMIT 5");
}
...
<div class="row g-3 mb-4">
    ...
    <div class="col-sm-4">
        <a href="job_applications.php" class="stat-card">
            <div class="stat-icon" style="background:rgba(99,102,241,.1);color:#6366f1;">
                <i class="fas fa-briefcase"></i>
            </div>
            <div>
                <div class="stat-value"><?= $job_count ?><?= $job_new > 0 ? ' <span style="font-size:13px;color:#ef4444;">(' . $job_new . ' new)</span>' : '' ?></div>
...
<div class="row g-3">
    ...
    <div class="col-lg-12">
        <div class="pl-card">
            <div class="pl-card-header">
                <h5><i class="fas fa-briefcase me-2" style="color:#6366f1"></i>Recent Job Applications</h5>
                <a href="job_applications.php" style="font-size:13px;color:#f07b14;text-decoration:none;font-weight:600;">View All →</a>
            </div>
            <div class="pl-card-body">
                <table class="pl-table">
                    <thead><tr><th>Applicant</th><th>Position</th><th>Status</th><th>Date</th></tr></thead>
                    <tbody>
                    <?php
                    $sc = ['new'=>'#ef4444','reviewed'=>'#3b82f6','shortlisted'=>'#10b981','rejected'=>'#9ca3af'];
                    while ($r = $recent_jobs->fetch_assoc()): ?>
                    <tr>
                        <td><strong><?= h($r['full_name']) ?></strong></td>
                        <td style="color:#6b7280"><?= h($r['position']) ?></td>
                        <td><span style="font-size:11.5px;font-weight:700;color:<?= $sc[$r['status']] ?? '#374151' ?>;">● <?= ucfirst($r['status']) ?></span></td>
                        <td style="color:#9ca3af;font-size:12px"><?= date('d M Y', strtotime($r['created_at'])) ?></td>
                    </tr>
                    <?php endwhile; ?>
                    </tbody>
                </table>
